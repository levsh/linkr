...
class TestRpcModels:
    ...
    def test_request_with_args(self):
        req = Request(method="foo", args=(1,), kwds={"x": 2})
        assert req.method == "foo"
        assert req.args == (1,)
        assert req.kwds == {"x": 2}

    def test_request_with_headers(self):
        req = Request(method="test", args=(), kwds={}, headers={"routing_key": "test", "timeout": 5.0})
        assert req.headers["routing_key"] == "test"
        assert req.headers["timeout"] == 5.0

    def test_request_serialization(self):
        req = Request(method="test", args=(), kwds={"key": "value"})
        data = req.model_dump_json()
        restored = Request.model_validate_json(data)
    ...
    def test_response_defaults(self):
        req = Request(method="ping", args=(), kwds={})
        resp = Response(id=req.id, type="result", data=None)
    ...
    def test_response_serialization(self):
        req = Request(method="ping", args=(), kwds={})
        resp = Response(id=req.id, type="result", data={"result": 42})
        data = resp.model_dump_json()
        restored = Response.model_validate_json(data)
    ...
    def test_response_invalid_id(self):
        with pytest.raises(ValidationError):
            Response(id="not-a-uuid")  # type: ignore[arg-type]
